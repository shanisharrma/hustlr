...
import { ISearchResult, ISellerGig } from '@shanisharrma/hustlr-shared';
...
const moreLikeThis = async (req: Request, res: Response): Promise<void> => {
  ...
  res.status(StatusCodes.OK).json({ message: 'More gigs like this results', total: gigs.total, gigs: resultHits });
};
