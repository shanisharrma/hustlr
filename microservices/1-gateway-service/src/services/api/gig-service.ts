...
import { ISellerGig } from '@shanisharrma/hustlr-shared';

export let axiosGigInstance: ReturnType<typeof axios.create>;

class GigService {
  constructor() {
    const axiosService: AxiosService = new AxiosService(`${serverConfig.GIG_BASE_URL}/api/v1/gig`, 'gig');
  ...
  async getSelleGigs(sellerId: string): Promise<AxiosResponse> {
  ...
  async getSellerInactiveGigs(sellerId: string): Promise<AxiosResponse> {
    const response: AxiosResponse = await axiosGigInstance.get(`/seller/inactive/${sellerId}`);
    return response;
  }
  ...
  async updateGig(gigId: string, body: ISellerGig): Promise<AxiosResponse> {
    const response: AxiosResponse = await axiosGigInstance.post(`/${gigId}`, body);
    return response;
  }
  ...
  async updateActiveGigProp(gigId: string, active: string): Promise<AxiosResponse> {
