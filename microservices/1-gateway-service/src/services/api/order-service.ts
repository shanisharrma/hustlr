...
import { IDeliveredWork, IExtendedDelivery, IOrderDocument, IOrderMessage } from '@shanisharrma/hustlr-shared';

export let axiosOrderInstance: ReturnType<typeof axios.create>;

class OrderService {
  constructor() {
    const axiosService: AxiosService = new AxiosService(`${serverConfig.ORDER_BASE_URL}/api/v1/order`, 'order');
  ...
  async cancelOrder(paymentIntent: string, orderId: string, body: IOrderMessage): Promise<AxiosResponse> {
    const response: AxiosResponse = await axiosOrderInstance.put(`/cencel/${orderId}`, {
      paymentIntent,
      orderData: body
    });
    return response;
  ...
  async requestDeliveryExtenstion(orderId: string, body: IExtendedDelivery): Promise<AxiosResponse> {
