...
const logger: Logger = winstonLogger(`${serverConfig.ELASTIC_SEARCH_URL}`, 'orderServiceConsumer', 'debug');
...
export const consumeReviewFanoutMessages = async (channel: Channel): Promise<void> => {
  try {
    ...
    logger.log('error', 'OrderService consumer consumeReviewFanoutMessages() method error: ', error);
  }
};
