...
import { BadRequestError, IOrderDocument } from '@shanisharrma/hustlr-shared';
import { Request, Response } from 'express';
import { StatusCodes } from 'http-status-codes';
import Stripe from 'stripe';

const stripe: Stripe = new Stripe(serverConfig.STRIPE_SECRET_KEY!, { typescript: true });
...
const intent = async (req: Request, res: Response): Promise<void> => {
  ...
  if (customerId) {
    ...
    // for the purchase under $50, an additional $2 is applied
